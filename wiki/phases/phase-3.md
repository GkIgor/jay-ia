...
- **LLM Router Agnóstico (Multi-Modelos)**: Criação da interface `llm.Client` e do pacote `llm` para desacoplar a Jay de SDKs específicos (como o do Gemini). Desenvolvemos um **Plan Builder** no `LLMPlanner` que isola a montagem do plano dos detalhes do modelo.
- **Loop de Function Calling Multi-Turno**: Implementação no Daemon de um loop persistente com limites rígidos (`MaxPlanningIterations = 5`) e uso de um `ConversationManager` dedicado em Go para retransmitir respostas de ferramentas de volta à IA até obter a conclusão textual final.
- **Testes Unitários de Integração**: Teste unitário completo em `daemon_test.go` validando o fluxo de bloqueio, emissão do prompt, resposta do mock cliente via IPC e desbloqueio seguro do Daemon, além de novas suítes de teste de router, conversa e planner.
